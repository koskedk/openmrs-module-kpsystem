...
kenyaemrApp.controller('ReportProfiling', ['$scope', '$http', function($scope, $http) {
	 ...
	 */
	$scope.init = function() {
		$http.get(ui.fragmentActionLink('kpsystem', 'developer/developerUtils', 'getReportProfilingEnabled')).
	...
	$scope.setEnabled = function(enabled) {
		$scope.results = [];
		$http.get(ui.fragmentActionLink('kpsystem', 'developer/developerUtils', 'setReportProfilingEnabled', { enabled: enabled })).
			success(function() {
...
kenyaemrApp.controller('PatientValidation', ['$scope', '$http', function($scope, $http) {
	...
	$scope.run = function() {
		$scope.results = [];
		$scope.loading = true;
		$http.get(ui.fragmentActionLink('kpsystem', 'developer/developerUtils', 'validatePatients', {})).
			success(function(data) {
				$scope.results = data;
				$scope.loading = false;
			});
...
kenyaemrApp.controller('GroovyConsole', ['$scope', '$http', '$window', function($scope, $http, $window) {
	...
	$scope.run = function() {
		...
		var actionUrl = ui.fragmentActionLink('kpsystem', 'developer/developerUtils', 'executeGroovy', { returnFormat: 'json' });
