...
kenyaemrApp.controller('ServerInformation', ['$scope', '$http', '$timeout', function($scope, $http, $timeout) {
	...
	$scope.refresh = function() {
		$http.get(ui.fragmentActionLink('kpsystem', 'system/systemUtils', 'getServerInformation')).
...
kenyaemrApp.controller('DatabaseSummary', ['$scope', '$http', function($scope, $http) {
	...
	$scope.init = function(appId) {
		...
		$http.get(ui.fragmentActionLink('kpsystem', 'system/systemUtils', 'getDatabaseSummary', { appId: $scope.appId })).
			success(function(data) {
				$scope.infos = data;
			});
	};
