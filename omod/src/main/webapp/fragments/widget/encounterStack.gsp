<%
	...
	if (config.encounters && config.encounters.size() > 0) {
		config.encounters.each { encounter ->
			...
			def form = encounter.form ? ui.simplifyObject(encounter.form) : [ iconProvider : "kpsystem", icon : "forms/generic.png" ]
...
<div class="ke-stack-item ke-navigable" onclick="${ onClick }">
	...
	${ ui.includeFragment("kpui", "widget/icon", [ iconProvider: form.iconProvider, icon: form.icon, useViewOverlay: true, tooltip: "View Encounter" ]) }
